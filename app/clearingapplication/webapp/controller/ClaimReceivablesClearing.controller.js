...
sap.ui.define([
     ...
     * @param {any} CRActionHelper - Utility object for CR Specific handling actions
	...
	Constants
        ...
        return BaseController.extend("atom.ui.clearing.clearingapplication.controller.ClaimReceivablesClearing", {
            formatter: formatter,
            util: util,
            Fragment: Fragment,
            ActionHelper: CRActionHelper,


            onInit: function () {
                this._initDisplayData();
                var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
                oRouter.getRoute("RouteClaimReceivablesClearingView").attachPatternMatched(this._onRouteMatched, this);
            },

            _initDisplayData: async function () {
                this.getView().setBusy(true);
                this._ClearingType = "ClaimReceivables";
                // todo: Add the clearing data to the model
                // this.getOwnerComponent().setModel(new JSONModel(MockData.SelModelMock),"clearingPageModel");
                let oClearingModel = this.getOwnerComponent().getModel("clearingPageModel");

                // oClearingModel.setData(MockData.SelModelMock);
                ...
                let aProcessingStatus = oData.SelectionResult.filter(line => line.TrType !== "Unallocated Insurance Monies").reduce(function (accumulator, item) {
                    // Create a unique key for each combination of OrigCurr
                    let uniqueKey = item.BpId + "|" + item.BpName + "|" + item.ExpPayCurr;
                ...
                CRActionHelper.setController({ name: "Clearing", controller: this });
                ...
                this._ClearingOPMetadataInfo = await util.getClearingOPMetadataInfo(this.getOwnerComponent().getModel("clearingApplicationModel"), "clearingapplicationService.CRClearingOP");
            ...
            onCheckBoxTransactionItemSelect: async function (oEvent) {
                ...
                if (oEvent.getParameter("selected")) {
                    await CRActionHelper.clearingItemSelected(oContext, that, oItem);

                } else {
                    await CRActionHelper.clearingItemUnSelected(oContext);
                }

                CRActionHelper.calculateClearingAmnDifference(that);
            ...
            onButtonUnSelectAllPress: async function (oEvent) {
                ...
                oContexts.forEach((oContext) => {
                    ...
                    if (Constants.ClearingStatusValidForClearing.includes(oObject.ClearingStatus) && oObject.isSelected === true) {
                        CRActionHelper.clearingItemUnSelected(oContext);
                    }
                });
            ...
            onButtonSelectAllPress: async function (oEvent) {
                ...
                try {
                    ...
                    await new Promise((resolve) => {
                        const processItemSelection = async () => {
                            for (const oContext of oContexts) {
                                const oObject = oContext.getObject();
                                if (Constants.ClearingStatusValidForClearing.includes(oObject.ClearingStatus) && oObject.isSelected === false) {
                                    await CRActionHelper.clearingItemSelected(oContext, that, oObject);
                                }
                            }
                            resolve();
                        };
            ...
            onAllocAmnChange: async function (oEvent) {
                ...
                await CRActionHelper.onAllocAmnChange(oContext, that);
                await CRActionHelper.calculateClearingAmnDifference(that);
            ...
            onAddPaymentButtonPress: function (oEvent) {
                let oModel = this.getView().getModel(),
                    ...
                    oBPDetail = aSelectionResult.find(x=> x.BpName !== "");
                ...
                // @ts-ignore
                oPaymentBCToBeAdded.BpName = oBPDetail.BpName;
                // @ts-ignore
                oPaymentBCToBeAdded.BpId = oBPDetail.BpId;
                // @ts-ignore
            ...
            onExecuteClearingButtonPress: function (oEvent) {
                ...
                if (
                    !aClearingOP.some(x => x.isSelected === true && Constants.PayableTransactionTypesInClaimReceivables.includes(x.TrType))
                    || !aClearingOP.some(x => x.isSelected === true && !Constants.PayableTransactionTypesInClaimReceivables.includes(x.TrType))) {
                    ...
                    if (!util.checkPostingDateOnCurrentMonth(sPostingDate.substring(0, 10))) {
                        MessageBox.warning(this.getView().getModel("i18n").getResourceBundle().getText("PostingDateInOtherMonth"), {
                            ...
                            onClose: async (sAction) => {
                                ...
                                await CRActionHelper.tryProceedToClear(oEvent);
                                // this.getView().setBusy(false);
                                oEvent.getSource().getParent().setBusy(false);
                            }
                        });
            ...
            onConfirmToSaveButtonTaxPairPress: async function (oEvent) {
                oEvent.getSource().getParent().setBusy(true);
                try {
                    await CRActionHelper.saveSOADetails();
                } catch (error) {
                ...
                if (!this._oCopySOARefDialog) {
                    this._oCopySOARefDialog = new sap.m.Dialog({
                        ...
                        beginButton: new sap.m.Button({
                            ...
                            enabled: false,
                            press: function () {
                                CRActionHelper.copySOAReference(Element.getElementById("soaRefInput").getValue());
                            }
                        }),
 ...
 */
                    ...
                    this._oClearSOARefDialog = new sap.m.Dialog({
                        ...
                        beginButton: new sap.m.Button({
                            ...
                            press: async function () {
                                CRActionHelper.clearSOAReference(Element.getElementById("soaRefInputToClear").getValue());
                            }
                        }),
                        endButton: new sap.m.Button({
                            text: "Cancel",
            ...
            _constructFilterForClearingTable: function () {
                const aColumnsToBeFiltered = Constants.filterColumnsForClaimReceivables;
                let oModel = this.getView().getModel();
            ...
            onRoeRecCurrInputChange: async function (oEvent) {
                ...
                let oTable = this.getView().byId("idClearingTable");
                oTable.setBusy(true);
                await CRActionHelper.onRoeRecCurrInputChange(oContext);
                await CRActionHelper.calculateClearingAmnDifference(that);
                oTable.setBusy(false);
