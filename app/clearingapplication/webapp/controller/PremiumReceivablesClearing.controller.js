...
sap.ui.define([
    ...
    function (BaseController,
        ...
        return BaseController.extend("atom.ui.clearing.clearingapplication.controller.PremiumReceivablesClearing", {
            formatter: formatter,
            util: util,
            Fragment: Fragment,
            Constants: Constants,
            ...
            onInit: function () {
                ...
                oRouter.getRoute("RoutePremiumReceivablesClearingView").attachPatternMatched(this._onRouteMatched, this);
            },

            _initDisplayData: async function () {
                this.getView().setBusy(true);
                let oClearingModel = this.getOwnerComponent().getModel("clearingPageModel");
                this._ClearingType = "PremiumReceivables";
            ...
            */
            onExecuteClearingButtonPress: function (oEvent) {
                ...
                if (!aClearingOP.find(x => x.isSelected === true)) {
                    MessageBox.information(oView.getModel("i18n").getResourceBundle().getText("NoTransactionsSelected"));
                    return;
                }
                if (!(aClearingOP.some(x => x.isSelected === true && Constants.PayableTransactionTypesInPremiumReceivables.includes(x.TrType))
                    || !aClearingOP.some(x => x.isSelected === true && !Constants.PayableTransactionTypesInPremiumReceivables.includes(x.TrType)))) {
                    MessageBox.information(oView.getModel("i18n").getResourceBundle().getText("NoTransactionsSelectedForAction"));
                    return;
                }
 ...
 */
            ...
            _constructFilterForClearingTable: function () {
                const aColumnsToBeFiltered = Constants.filterColumnsForPremiumReceivables;
                let oModel = this.getView().getModel();
